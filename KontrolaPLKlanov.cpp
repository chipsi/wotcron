/*
*   Hladanie PL klanov. (Prerobene z CS klanov)
*   Najprv vyberie hracov ktory pouzivaju v hre PL klienta a ich clan_id
...
>>>>>>> d0d07bebbcc18d6fa46370aa0cc1dddf716eab42
    ...
    PGresult* SelectClanAll()
    {
        ...
        const char *sql;
        sql = "SELECT DISTINCT(clan_id) FROM players_all pa LEFT JOIN players_info pi ON pa.account_id = pi.account_id WHERE client_language = 'pl'";
        res = PQexec(conn , sql);
    ...
    PGresult* KlientCS()
    {
        ...
        string string_sql;
        string_sql = "SELECT pa.clan_id, pa.account_id FROM players_all pa LEFT JOIN players_info pi ON pa.account_id = pi.account_id  WHERE client_language = 'pl' AND pa.clan_id > 0";
    ...
    void InsertClansCS(string clans)
    {
        ...
        truncate = "TRUNCATE tmp_clans_pl";
        sql = "INSERT INTO tmp_clans_pl VALUES "+clans ;
...
// Funkcia ktora spocita v resulte pocet hracov s PL klientom
...
int main()
{
    ...
    // Natiahnem si do pamate vsetkych hracov a ich clan_id ktory maju PL klienta
    ...
    // Deklarujem funkciu na spocitavnie hracov PL z tabulky players_info
    ...
    for(i = 0; i < ntuples; i++)
    {
         ...
         /* Spocitam pocet hracov ktory hraju s PL klientom v pamati pocitaca*/
    ...
    cout << endl << "*********** Kontrola PL klanov ****************" << endl;
    cout << "Pocet prehladavanych klanov: \t" << ntuples << endl;
    cout << "Pocet PL klanov: \t\t" << pocet_cs_klanov << endl;
    cout << "Cas: \t \t\t\t" << secs << endl ;
    cout << endl;
