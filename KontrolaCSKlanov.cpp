/*
*   Hladanie Cz/Sk klanov
*   Najprv vyberie hracov ktory pouzivaju v hre CS klienta a ich clan_id
*   Potom zacne pozerat jednotlive klany a spocitavat kolko percent hracov
*   v klane pouziva CS klienta
*
*   24.07.2016 Boris Fekar
*/
...
class Databaza
{
    ...
    PGresult* SelectClanAll()
    {
        ...
        const char *sql;
        sql = "SELECT DISTINCT(clan_id) FROM players_all pa LEFT JOIN players_info pi ON pa.account_id = pi.account_id WHERE client_language = 'cs'";
        res = PQexec(conn , sql);
    ...
    PGresult* KlientCS()
    {
        ...
        string string_sql;
        string_sql = "SELECT pa.clan_id, pa.account_id FROM players_all pa LEFT JOIN players_info pi ON pa.account_id = pi.account_id  WHERE client_language = 'cs' AND pa.clan_id > 0";
    ...
    void InsertClansCS(string clans)
    {
        ...
        truncate = "TRUNCATE tmp_clans_cs";
        sql = "INSERT INTO tmp_clans_cs VALUES "+clans ;
        i_clans = "insert into clans (select * from tmp_clans_cs where clan_id not in (select clan_id from clans))";
        
        conn = this->Connect();
        PQexec(conn , truncate.c_str());
        PQexec(conn , sql.c_str());
        PQexec(conn , i_clans.c_str());
        PQfinish(conn);
    }
...
int main()
{
    ...
    // Natiahnem si do pamate vsetkych hracov a ich clan_id ktory maju CS klienta
    ...
    // Deklarujem funkciu na spocitavnie hracov CS z tabulky players_info
    ...
    cout << endl << "*********** Kontrola CS klanov ****************" << endl;
    cout << "Pocet prehladavanych klanov: \t" << ntuples << endl;
    cout << "Pocet CS klanov: \t\t" << pocet_cs_klanov << endl;
    cout << "Cas: \t \t\t\t" << secs << endl ;
    cout << endl;
