...
void GetPlayers(mymap *a_ids, int *ntuples)
{
    ...
    const char query[] = "SELECT account_id FROM cz_players";
...
void PreparedStatmentPlayersStat(PGconn *conn)
{
    ...
    const char query[]  = "SELECT damage_dealt,spotted,frags,dropped_capture_points,wins,battles FROM players_stat_all WHERE account_id = $1 ";
